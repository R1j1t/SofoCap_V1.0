...
var invokeChaincode = function(peerNames, channelName, chaincodeName, fcn, args, username, org) {
	...
	var tx_id = null;
	console.log('==================='+username+' is doing transaction from '+org+'====================');
	return helper.getRegisteredUsers(username, org).then((user) => {
		...
		errorMessage=proposalResponses[0];       //error message given
		...
		if (response.status === 'SUCCESS') {
			...
			logger.error('Check This- ' + response)
			if (!response.status){
				logger.error('Failed to order the transaction. Error code: ' + errorMessage);
				return 'Failed to order the transaction. Error code: ' + errorMessage;
			}else {
